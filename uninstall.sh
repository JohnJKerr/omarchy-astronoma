...
# Prefer the installed copy: it is the one whose row is actually in the menu.
# Fall back to this checkout so an already-removed plugin can still be
# tidied up after the fact.
MENU_ENTRY="$TARGET_DIR/bin/astronoma-menu-entry"
[[ -x $MENU_ENTRY ]] || MENU_ENTRY="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/bin/astronoma-menu-entry"
